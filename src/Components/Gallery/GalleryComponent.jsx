...
function GalleryComponent(item) {
    ...
    return (
        <>
            <div className='hovertag cursor-pointer flex flex-col items-center' style={{ width: '370px', marginRight: '8rem', minWidth: '400px' }} onClick={() => {
                setIsOpen(true);
            }}>
                <div className='gallerytag flex justify-center'>
                    <img className='imgbg' src={Gallery_bg} />
                    <img className='self-center' style={{ height: '80%', position: 'absolute' }} src={element.imageURL} alt='thumbnail' />
                </div>
            ...
            {isOpen && (
                <ExpandedGallery show={isOpen} onClickOutside={() => setIsOpen(false)} pictures={element.pictures} />
            )}
        </>
    )
}
